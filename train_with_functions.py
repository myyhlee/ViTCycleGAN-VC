...
def train_fn(disc_Y, disc_X, gen_X, gen_Y, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler):
    ...
    for idx, (x, y) in enumerate(loop):
        ...
        if idx % 100 == 0:
            save_pickle_file(variable = fake_x, fileName = os.path.join("/home/yuholee/develop/ViTCycleGAN_Mel/saved_mels", f"{idx}_mel_x.pickle"))
            # save_pickle_file(variable = fake_x *0/5 + 0.5, fileName = os.path.join("saved_mels", f"{idx}_mel_x.pickle"))

            save_pickle_file(variable = fake_y, fileName = os.path.join("/home/yuholee/develop/ViTCycleGAN_Mel/saved_mels", f"{idx}_mel_y.pickle"))
